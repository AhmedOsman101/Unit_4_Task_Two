<div class="bg-white dark:bg-gray-800 w-full min-h-screen flex justify-center items-center">
    <div class="text-center w-full mx-auto py-12 px-4 sm:px-6 lg:py-16 lg:px-8">
        <h2 class="text-3xl font-extrabold text-black dark:text-white sm:text-4xl pt-10">
            <span class="block">
                Are you ready to compete?
            </span>
            <span class="block text-indigo-500">
                Join the college tournament!
            </span>
        </h2>
        <p class="text-xl mt-4 max-w-md mx-auto text-gray-400">
            Showcase your skills in a variety of events and claim victory. Whether you're an individual competitor or
            part of a team, this is your chance to shine.
        </p>
        <div class="lg:mt-0 lg:flex-shrink-0">
            <div class="mt-12 inline-flex rounded-md shadow">
                <button type="button"
                    class="py-4 px-6 bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-500 focus:ring-offset-indigo-200 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 rounded-lg">
                    <a href="{{route('events')}}">Register Now</a>
                </button>
            </div>
        </div>
    </div>
</div>
